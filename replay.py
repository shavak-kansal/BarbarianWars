...
class GameLoop():
    ...
    def runGame(self, inputArr):
        ...
        while True:
            ...
            while (index < (len(inputArr)-1)) and (int(inputArr[index][2:]) <= self.realWorldTime):
                currentInput.append(inputArr[index][0])
                index+=1
...
with open('replays/replays.txt', 'r') as f:

    itr = 0
    for line in f:
        itr += 1
        replayList.append(line)

        replayString = str(itr) + " : " + line

        nextline = next(f)
        if(nextline == "W\n"):
            replayString += ' - Victory\n'
        else:
            replayString += ' - Defeat\n'
...
print("Choose the replay you want to display: ")
replayChoice = input("Enter your choice: ")

replayChoice = int(replayChoice)

inputArr = replayList[replayChoice-1].split(".")
kingChoice = playerChoices[replayChoice-1]

system('clear')

Game = GameLoop(kingChoice)
Game.runGame(inputArr)
